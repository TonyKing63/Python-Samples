...
##------------- Init Subwindows
    ...
    def MakeEditorWindow(self, win, log):
        self.edl = FrogEditor(
            win, -1, style=wx.SUNKEN_BORDER, statusBar=self.sb)
...
# override this to add more menus
    ...
    def AddFileMenu(self, menu):
        fileMenu = wx.Menu()
        self.AddMenuItem(fileMenu, '&New File\tCtrl-N', 'New File',
                         self.OnNewFile)
        self.AddMenuItem(fileMenu, '&Open File\tCtrl-O', 'Open File',
                         self.OnOpenFile)
        self.AddMenuItem(fileMenu, '&Save File\tCtrl-S', 'Save File',
                         self.OnSaveFile)
        self.AddMenuItem(fileMenu, 'Save File &As\tCtrl-A', 'Save File As',
                         self.OnSaveFileAs)
    ...
    def AddEditMenu(self, menu):
        editMenu = wx.Menu()
        self.AddMenuItem(editMenu, 'Cut\tCtrl-X', 'Cut',
                         self.edl.OnCutSelection)
        self.AddMenuItem(editMenu, '&Copy\tCtrl-C', 'Copy',
                         self.edl.OnCopySelection)
        self.AddMenuItem(editMenu, 'Paste\tCtrl-V', 'Paste', self.edl.OnPaste)
        self.AddMenuItem(editMenu, 'Edit preferences', 'Edit Preferences',
                         self.OnEditPreferences)
        menu.Append(editMenu, 'Edit')

    def AddHelpMenu(self, menu):
        helpMenu = wx.Menu()
        self.AddMenuItem(helpMenu, 'About', 'About the program',
                         self.OnHelpAbout)
...
##---------------- Init Buttons
    ...
    def AddButtons(self, window, container):
        buttonPos = None
        buttonSize = None
        self.NewButton(window, container, "New", buttonPos, buttonSize,
                       self.OnNewFile)
        self.NewButton(window, container, "Open", buttonPos, buttonSize,
                       self.OnOpenFile)
        self.NewButton(window, container, "Save", buttonPos, buttonSize,
                       self.OnSaveFile)

##-------------- Init Dialogs

    def MessageDialog(self, text, title):
        messageDialog = wx.MessageDialog(self, text, title,
    ...
    def SelectFileDialog(self,
        ...
        if wildCard == None:
            wildCard = "*.*"
        fileName = None
        fileDialog = wx.FileDialog(self, "Choose a file", defaultDir,
...
##---------------- Event handlers
    ...
    def OnNewFile(self, event):
        if self.edl.BufferWasTouched():
            if not self.OkCancelDialog("New file - abandon changes?",
                                       "New File"):
                return
        self.NewFile()
        self.edl.SetFocus()
    ...
    def OnOpenFile(self, event):
        if self.edl.BufferWasTouched():
            if not self.OkCancelDialog("Open file - abandon changes?",
                                       "Open File"):
                return
    ...
    def OnFileExit(self, event):
        if self.edl.BufferWasTouched():
            if not self.OkCancelDialog("Exit program - abandon changes?",
                                       "Exit"):
                return
        self.OnCloseWindow(event)

    def OnEditPreferences(self, event):
        self.MessageDialog("Edit preferences is not implemented yet.",
                           "Not implemented.")
        pass
